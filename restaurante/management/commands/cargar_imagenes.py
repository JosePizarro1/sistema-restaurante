...
def download_image(url: str, timeout: int = 15) -> bytes | None:
    ...
    headers = {'User-Agent': 'Mozilla/5.0 (compatible; RestoPOS/1.0; +http://localhost)'}
    try:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if failed > 0 and not dry_run:
            self.stdout.write(self.style.WARNING('Some dishes failed. You can re-run with --dish <name> for specific dishes.'))
